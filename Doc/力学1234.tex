...
\begin{enumerate}[leftmargin=18pt, labelsep=10pt, labelsep=10pt, itemindent=9pt]
	\item[\f{例}] 原点を共通に持つ2つの座標系の軸が$\bunsuu{\pi}{4}$の角をつくっている．
		...
		\begin{enumerate}[label=(\arabic*), labelsep=10pt, leftmargin=23pt]
			...
			\begin{equation}
				x' = \mathrm{OB} + \mathrm{OA'} + \mathrm{A'B} = \mathrm{OA}\cos\bunsuu{\pi}{4} + \mathrm{AP}\sin\bunsuu{\pi}{4} = \bunsuu{1}{\sqrt{2}}(x + y) \label{equ:mecha1-1}
			\end{equation}
			また
			\begin{equation}
				y' = \mathrm{AP}\cos\bunsuu{\pi}{4} - \mathrm{OA}\sin\bunsuu{\pi}{4} = \bunsuu{1}{\sqrt{2}}(-x + y) \label{equ:mecha1-2}
			\end{equation}
			式(\ref{equ:mecha1-1})，式(\ref{equ:mecha1-2})が$x',\ y'$を$x,\ y$で表す式である．$x,\ y$について解けば
			\begin{gather}
